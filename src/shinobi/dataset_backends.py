...
SHARED_STORAGE_QUALIFICATION_PROFILE = "slurm-shared-storage/v1"
...
class SharedStorageQualification(BaseModel):
    """Persisted evidence that one exact Slurm storage tree passed M2.

    This record is supplied by the site after running the physical probe. It
    is not generated by submission preparation: the compiler and every
    compute worker only validate and pin its exact bytes.  It is an operator
    attestation guarded by filesystem permissions, not a signed artifact:
    nothing here can tell a file the M2 checker wrote from one written by
    hand, so protect it as you would the site configuration.
    """

    model_config = ConfigDict(extra="forbid", frozen=True)

    schema_version: Literal[1] = 1
    profile: Literal["slurm-shared-storage/v1"] = SHARED_STORAGE_QUALIFICATION_PROFILE
    ...
    def _valid_evidence(self) -> "SharedStorageQualification":
        if not self.storage_root.is_absolute():
            raise ValueError("qualified shared-storage root must be absolute")
        if self.verified_at <= 0:
            raise ValueError("shared-storage qualification needs a positive verification timestamp")
        if len(self.workers) < 2 or len(self.workers) != len(set(self.workers)):
            raise ValueError("shared-storage qualification needs at least two unique workers")
        if not self.evidence.strip():
            raise ValueError("shared-storage qualification needs physical-probe evidence")
        return self


def load_shared_storage_qualification(path: Path) -> tuple[SharedStorageQualification, str]:
    """Read and fingerprint one site-issued shared-storage qualification."""

    path = path.resolve()
    if not path.is_absolute() or not path.is_file():
        raise DatasetLifecycleUnavailableError(f"shared-storage qualification is not a readable absolute file: {path}")
    try:
        payload = path.read_bytes()
        qualification = SharedStorageQualification.model_validate_json(payload)
    except (ValueError, OSError) as exc:
        raise DatasetLifecycleUnavailableError(f"invalid shared-storage qualification {path}: {exc}") from exc
    root = qualification.storage_root.resolve()
    if path != root and not path.is_relative_to(root):
        raise DatasetLifecycleUnavailableError(f"shared-storage qualification {path} is outside its qualified root {root}")
    qualification = qualification.model_copy(update={"storage_root": root})
    return qualification, hashlib.sha256(payload).hexdigest()
...
def worker_dataset_backend_capability(*, mutation: bool, qualification: SharedStorageQualification | None = None) -> DatasetBackendCapability:
    ...
    if qualification is None:
        return DatasetBackendCapability(
            ...
            inspection_location="unqualified",
            recovery_location="unqualified",
            reason="no persisted site qualification proves shared identity and distributed metadata semantics for this storage tree",
        )
    return DatasetBackendCapability(
        backend="slurm-worker",
        lifecycle=lifecycle,
        ...
        reason=(
            f"site qualification {qualification.qualification_id} covers {qualification.storage_root}; "
            "the immutable plan records an identity mapping for that shared storage; "
...
__all__ = [
    ...
    "SHARED_STORAGE_QUALIFICATION_PROFILE",
