...
def check_offloadable(recipe: "Recipe", *, worker: bool = False) -> RecipeGraph:
    ...
    root_datasets = sorted(dataset_fields(recipe.inputs_model) | dataset_fields(recipe.outputs_model))
    if root_datasets:
    ...
    for ref in recipe.steps:
        scope = ref.step
        strict_datasets = sorted(dataset_fields(scope.inputs_model) | dataset_fields(scope.outputs_model))
        if strict_datasets:
