...
Change detection is `(relative_path, mtime_ns, size)` per file, not a full
content hash: radio-astronomy inputs (MS directories, FITS cubes) can be
many GB, and byte-for-byte hashing every input on every run to decide
whether to skip would defeat the point -- this is the same tradeoff Make
itself accepts. A path that is *also* a declared output field of the same
step is excluded from the input-hash contribution entirely: a dominant
pattern among real cabs/pysteps is mutating a path in place (e.g. a
flagging or calibration step reading and writing the *same* MS) -- if
that shared path's own post-mutation mtime were part of the input hash,
the *first* step of a resumed chain would always look "changed" (because
the previous run's later steps already mutated it), cascading into a full
rerun of everything downstream, exactly what caching exists to avoid. For
such a step, "unchanged" means its resolved params/config are unchanged
and its declared output still exists -- not that the shared path's mtime
hasn't moved.
...
def compute_cache_key(scope: Scope, func: Callable | None, prepared: dict[str, Any]) -> str:
    """Hashes `(scope.image, _identity(scope, func), canonicalized
    prepared params)`. Path-valued input fields are hashed by
    `_hash_path` (mtime+size, not content) unless the same field name is
    also a declared *output* field of this step (the in-place-mutation
    ...
    for name in sorted(prepared):
        value = prepared[name]
        if name in input_paths and name not in mutated_paths and value is not None:
