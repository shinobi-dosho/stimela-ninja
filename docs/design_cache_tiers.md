...
- **v5 → v6 (M2 metadata foundation).** `CacheManifest` and `ChainJournal` now share `storage.JsonFileStore`: a persistent sibling inode takes a Linux open-file-description lock for the complete read-modify-write transaction and also carries an append-only checksummed transaction log. OFD ownership avoids POSIX locks' "close any descriptor" release rule and independently opened stores conflict without a fragile path-keyed thread registry; an at-fork descriptor registry prevents a child extending the parent's lock lifetime. OFD locks conflict with the ordinary POSIX locks used by NFSv4. The log is the commit point; an incomplete trailing record is recoverable, while a completed corrupt record fails closed. The ordinary JSON is then refreshed through a random `O_EXCL` same-directory temporary, file fsync, atomic replacement and immediate-parent fsync; failure after the log commit warns rather than falsely reporting a failed update. Making replacement only a materialized compatibility view is required by the physical Kudu/Nyala topology: direct lock exclusion worked, but Kudu's local-ext4 view could still reopen a stale path after Nyala's NFS replacement and lose updates. The first update imports the unchanged legacy JSON shape, but this is a one-way upgrade and every later writer must use the M2 protocol. New views and locks use collaborative umask/default-ACL-filtered permissions, existing logs can be read without write access, and lock waits time out diagnostically after 60 seconds. The additive lock/log inode must never be unlinked: `ninja clean --cache` resets under it and supplies corrupt-log recovery while leaving the lock domain intact. Pre-commit system-call failures are fatal rather than degrading to an unsafe write. A local process cannot prove that a filesystem honours locks across nodes, so shared-cluster support is gated on the parameterizable physical `tests/slurm_physical/run_m2.py` probe passing on that exact mount. The strengthened probe passed on Kudu/Nyala on 2026-09-13 (jobs 65--67), then passed twice after review hardening on 2026-09-14 (jobs 116--118 and 119--121): every node held in turn and excluded both peers, all 60 synchronized updates survived in each store, and materialized JSON agreed with log replay. This is deliberately only metadata serialization: ownership of a scientific path across a mutating tool's execution remains separate, and worker cache decisions/upstream identity remain the next M2 layer.
- **v6 → v7 (M2 worker identity).** Detached workers now make the cache decision inside their allocation. Explicit `OutputRef` inputs recover producer keys and producer fields only from committed parent attempt records; ordering-only and inferred mutation dependencies do not manufacture provenance. Frozen pystep code, pinned image digests and resolved tool-venv fingerprints enter the same `compute_cache_key` path used locally. Cache hits validate declared products, publish a new immutable attempt record, retain image/venv provenance, and preserve venv-backed runs' unpinned status. A changed branch invalidates only its wired descendants; worker-source packaging is intentionally excluded. Compiled cache overrides are frozen in the bundle instead of being dropped. M3 still owns scientific-path safety: worker leaves declaring in-place mutation execute uncached and snapshot recovery stays off until workspace ownership exists. The strengthened Kudu/Nyala worker-cache probe passed on 2026-09-13 (scientific jobs 88--114): real native, Apptainer and tool-venv steps hit on the unchanged detached run, upstream changes invalidated only wired descendants, and deleting one declared product reran only its producer.
- **v7 → v8 (M3 workspace ownership).** A write-declaring local or detached workflow now owns its canonical workspace through a durable root record plus a shared access registry. The claim is acquired before execution/submission, survives queued gaps and submitter death, and is released by local completion, terminal finalization, or explicit evidence-backed reconciliation. Local OFD liveness locks and Slurm accounting distinguish live, dead and uncertain owners; uncertainty, missing planned-job records and elapsed time never steal a reservation. The immutable execution plan makes detached ownership mandatory and records the registry plus canonical accesses. Statically known writable targets derive their authority from canonical target parents, so different launch workdirs and symlink spellings converge; the registry makes nested exclusion order-independent and catches cross-authority read/write overlap while allowing disjoint claims. Slurm requeues derive and publish a fresh attempt UUID from the planned seed plus `SLURM_RESTART_COUNT`, and downstream/finalizer reads follow the newest invocation. Worker mutation snapshots remain the next integration slice.
- **v8 → v9 (M3 worker mutation recovery).** Detached mutation-declaring leaves now enter the same `SnapshotGuard` used locally, including image- and venv-backed pysteps. Recovery before the cache decision is restricted to the concrete mutation paths of the allocation; inferred Slurm mutation dependencies make those paths exclusive while independent branches continue concurrently. An in-flight marker names the immutable final attempt record for that invocation. Rule B and the journal commit precede publication of that sole success oracle; the reusable cache index follows it, and trash disposal plus marker clearing remain last. A missing/failed attempt rolls back and invalidates a same-key stale cache entry, whereas a committed uncached mutator is preserved. Changed mid-chain params or bundled pystep code restore the state named by upstream provenance before rerunning. Fault injection covers S1--S5 plus the worker-result/cache-index boundary, spanning both pre-commit retry and post-commit cleanup recovery. The supported-release gate (`tests/slurm_physical/run_m3.py`) passed on the three-node Kudu/Nyala topology on 2026-09-18 (jobs 344--421): native/image/venv execution, overlapping metadata commits, selective invalidation, staged-source isolation, missing output/environment handling, cancelled finalization, physical S2/W_RESULT crashes, and real image/venv Slurm requeues all recovered exact content on the shared ext4/NFS view.

- **v4 → v5 (implementation).** Tier 1 ships as `src/shinobi/snapshots.py` + `src/shinobi/clonefs.py`, with `ProvenanceKey`, shared `storage.JsonFileStore`, the content sample in `cache.py`, and `mutated_path_fields` in `steps/schema.py`. Of v4's three load-bearing claims, one held and two had holes; three further defects turned up during implementation. In order of severity:
...
- **v3 → v4 (review round 2).** Review verified every §2 spine claim against the tree and found them all accurate — except one, and it was load-bearing. Four correctness holes in v3's own mechanisms, plus the spine correction:
  ...
  Smaller: generation-0 fingerprint names are hashed before filesystem use; the journal shares `storage.JsonFileStore` with `CacheManifest` (DRY); `run_id` minted per top-level dispatch (trash was `<pid>` in v3 — pids recycle); journal identity gains `st_ctime_ns` against inode reuse; the content sample uses stdlib `blake2b`, not a new blake3 dependency; the capability probe records its decisions for `ninja cache check`; snapshot/restore shell-outs are exec-form argv per SECURITY.md; the post-success sequence gets fault-injection points for tests; §5's path invariant carries the Rule-B-preflight caveat v3 omitted.
...
1. **The marker names one explicit success oracle (§4.2.10).** Local reconciliation checks the run-stamped manifest entry; detached reconciliation checks the immutable final attempt record named by the marker. Find a crash window where that oracle still mis-decides — bounded pre-commit re-execution is acceptable; a false *hit* or rollback of a committed uncached mutation is not.
...
**Detached-worker scope.** The opt-in worker compiler executes `_dispatch` inside each Slurm allocation, so binary cabs and supported image-/venv-backed pysteps participate in Tier 1. The legacy direct-command compiler still has no worker lifecycle and therefore no mutation recovery.
...
- **A3 (framework sees writes) — true for local dispatch and worker allocations.** All hooks live in `_dispatch`; the legacy direct-command compiler remains outside them.
...
The journal and snapshot payloads are one unit: created and cleared together (`ninja clean --cache` resets the journal and removes its states while holding the journal's persistent lock; a partial clean is worse than either alone, because generation 0's name is re-derived from live disk state and would then name the post-chain state under the position the base state used to hold). The lock inode and containing directories deliberately survive cleanup.

**Mechanics:** the journal gets `CacheManifest`'s exact discipline through shared `storage.JsonFileStore`: OFD-locked read-modify-write, a checksummed append-only commit log, and a replaceable JSON compatibility view. Two private copies of this rule would be the `_modelgen` drift waiting to happen. Per-path serialisation against concurrent mutators is inherited from "the declared graph is the truth" (cache.py:70–76): two steps mutating one path without an edge between them are already an unprotected race at `max_workers > 1`; with an edge, they never run concurrently.
...
- **S3.** Publish the success oracle: `manifest.record` for local dispatch, or the immutable final attempt record for a detached worker. The worker updates the reusable manifest index only after its attempt record is durable; an index failure costs a later rerun but cannot revoke committed work. (When Appendix A lands, store insert follows the same oracle boundary.)
- **S4.** Delete trash.
...
- **The marker's oracle committed this invocation** → the step completed (crash in S4/S5): delete trash if present and clear the marker. For local dispatch this means the manifest has the same `(step_path, cache_key, run_id)`; for a worker it means the named final attempt record is committed and matches the marker, including a legitimate `cache_key=None` uncached mutation. The tip snapshot is guaranteed by S1-before-S3.
- **The oracle is absent, failed or mismatched** → the step did not complete: remove a same-key stale cache entry, swap back trash if present, **mark the head untrusted**, and clear the marker. The next restore takes branch 1. If the crash was between S2 and S3, this may re-run a step whose tool returned zero but whose result never committed — bounded waste chosen over a false hit.
...
5. **The marker's result record is the success oracle.** Recovery never infers "the step finished" from journal state alone. Local markers use their run-stamped manifest entry; detached markers name their immutable final attempt record. Enforced by §4.2.10.
...
3. **Ownership uses one shared registry.** `chains.json`, `CacheManifest`, the root owner and the access registry use the v6 shared transaction contract, but only ownership persists across scientific execution. The registry permits disjoint claims and rejects canonical path equality/containment when either side writes; independent branches inside one workflow remain concurrent. It defaults to `~/.shinobi/workspace-owners.json` and can be placed on site-verified shared storage with `SHINOBI_OWNERSHIP_REGISTRY`. Bind-mount aliases remain undiscoverable.
...
6. **Re-run waste in the S2–S3 crash window.** Reconciliation treats a missing success record as "did not commit", so a crash between journal append and publication re-executes a tool that may already have returned zero. Narrow window, bounded cost, chosen over the false-hit alternative (invariant 5).
...
11. **The metadata transaction log is not compacted automatically yet.** Each update appends one top-level delta and every read replays the log while holding the lock. `ninja clean --cache` provides an explicit inode-preserving reset; in-place crash-safe compaction is follow-up performance work. Replacing the lock inode is forbidden, and replacing the materialized JSON cannot become the commit point on the physically verified mixed local/NFS topology.
...
2. ~~Journal placement~~ — **Resolved: separate file**, with the marker carrying `cache_key` (§4.2.3). Folding into the manifest would make S2+S3 one atomic write, at the price of the shipped format and a reserved key; the marker achieves the same disambiguation for less diff. Both files share `storage.JsonFileStore`.
...
/ `as_provenance_key` / `storage.JsonFileStore` / `CacheManifest.entry` /
...
2. **`JsonFileStore` factor-out** (§4.2.3). `CacheManifest` refactored onto it; no behaviour change; existing `test_cache.py` must pass unmodified.
