...
- **v5 → v6 (M2 metadata foundation).** `CacheManifest` and `ChainJournal` now share `storage.JsonFileStore`: a persistent sibling inode takes a Linux open-file-description lock for the complete read-modify-write transaction and also carries an append-only checksummed transaction log. OFD ownership avoids POSIX locks' "close any descriptor" release rule and independently opened stores conflict without a fragile path-keyed thread registry; an at-fork descriptor registry closes inherited lock descriptions in the child. OFD locks conflict with the ordinary POSIX locks used by NFSv4. The log is the commit point; an incomplete trailing record is recoverable, while a completed corrupt record fails closed. The ordinary JSON is then refreshed through a random `O_EXCL` same-directory temporary, file fsync, atomic replacement and directory-chain fsync. Making replacement only a materialized compatibility view is required by the physical Kudu/Nyala topology: direct lock exclusion worked, but Kudu's local-ext4 view could still reopen a stale path after Nyala's NFS replacement and lose updates. The target filenames and JSON shapes are unchanged, so old stores need no migration; their current state becomes the first logged snapshot. The additive lock/log inode must never be unlinked. System-call failures are fatal rather than degrading to an unsafe write. A local process cannot prove that a filesystem honours locks across nodes, so shared-cluster support is gated on the physical `tests/slurm_physical/run_m2.py` probe passing on that exact mount. The strengthened probe passed on Kudu/Nyala on 2026-09-13 (jobs 65--67): every node held in turn and excluded both peers, all 60 synchronized updates survived in each store, and materialized JSON agreed with log replay. This is deliberately only metadata serialization: ownership of a scientific path across a mutating tool's execution remains separate, and worker cache decisions/upstream identity remain the next M2 layer.
