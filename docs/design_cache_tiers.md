...
3. **Metadata transactions are multi-process; mutable data ownership is not.** `chains.json` and `CacheManifest` no longer lose read-modify-write updates between processes: both use the v6 shared lock/atomic-replacement contract. Cross-node use is supported only on a mount that passes the physical probe, because a successful local `flock` call cannot detect node-local locking. Nothing yet prevents two independent runs rewriting one MS at once; a path-scoped ownership lock held across the mutating window is a separate mechanism and remains future work. The whole journal is one safe but coarse transaction domain; sharding per chain is a contention optimization, not a correctness prerequisite. SQLite remains rejected (NFS/Lustre locking pathology).
